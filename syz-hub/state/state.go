...
func Make(dir string) (*State, error) {
	...
	// st.corpusSeq表示最大seq，seq通常可以用来表示连接上hub的最大个数
...
func (st *State) createManager(name string) (*Manager, error) {
	...
	// loadSeqFiled将corpusSeqFile中的文件转换为十进制的uint64类型返回
...
func (st *State) addInput(mgr *Manager, input []byte) {
	...
	// mgr.Corpus只写入corpus的sig
	mgr.Corpus.Save(sig, nil, 0)
	// 如果此程序没有在st.corpus全局数据库出现过，将其添加到全局数据库中
	// 判断有没有出现目前还有点问题，目前只能是输入的hash值一样才判断是一样
	if _, ok := st.Corpus.Records[sig]; !ok {
		st.Corpus.Save(sig, input, st.corpusSeq)
	}
}

// 删除属于全局数据库st.corpus，但不属于manager总和的corpus，可以视为不明corpus。
...
// 将filename中的文件转换为十进制的uint64类型返回
