...
func (r *randGen) generateCall(s *state, p *Prog) []*Call {
	...
	if s.ct == nil {
		...
		// 如果p.call为空，那么随机选择一个函数；否则从现有的函数中随机选择一个函数，
		// 根据这个函数的优先级列表随机生成函数；
		call := -1
		if len(p.Calls) != 0 {
...
func (r *randGen) myGenerateCall(s *state, p *Prog) []*Call {
	idx := 0
	if s.ct == nil {
		idx = r.Intn(len(r.target.Syscalls))
	} else {
		...
		if len(p.Calls) != 0 {
			// 选择最后一个函数
			call = p.Calls[len(p.Calls)-1].Meta.ID
		}
		// 全为0，重新选择函数
		if call != -1 && s.ct.run[call][len(r.target.Syscalls)-1] == 0 {
			call = -1
		}
		// 从选择的函数中选择下一个函数的下标
		idx = s.ct.MyChoose(r.Rand, call)
	}
	meta := r.target.Syscalls[idx]
	return r.generateParticularCall(s, meta)
}
