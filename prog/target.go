...
// 注册一个Target，对应文件sys/xxx/xxx(如sys/linux/amd64.go)中的不同init()函数
...
func GetTarget(OS, arch string) (*Target, error) {
	key := OS + "/" + arch
	target := targets[key]
	// 如果没有对应的target，输出支持的target和不支持的型号
	if target == nil {
...
func (target *Target) lazyInit() {
	...
	target.initTarget()
	//	initArch函数是RegisterTarget时传入的参数，在sys/(linxu)/init.go文件中
	target.initArch(target)
	target.ConstMap = nil // currently used only by initArch
}

func (target *Target) initTarget() {
	// target.ConstMap 静态常量
	// target.Consts 形如{Name: "ADDR_COMPAT_LAYOUT", Value: 2097152}
	target.ConstMap = make(map[string]uint64)
	for _, c := range target.Consts {
		target.ConstMap[c.Name] = c.Value
	}
	// ResourceDesc存入target.resourceMap中
	...
	// 找到能够产生这种resource的函数，并存放到target.resourceCtors中
