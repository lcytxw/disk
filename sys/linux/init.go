...
func initTarget(target *prog.Target) {
	...
	// 初始化MakeMmap和SanitizeCall，它们是两个函数，作用是创建一个mmap函数和对已经创建的函数
	// 参数进行修正
	target.MakeMmap = arch.makeMmap
	target.SanitizeCall = arch.sanitizeCall
	// SpecialTypes存放函数键值和对应的函数
...
// 创建一个mmap函数，传入参数为要映射的内存区域的起始地址和要映射的内存区域的大小
