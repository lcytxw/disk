...
func DefaultConfig() (*Config, *ExecOpts, error) {
	...
	// opts是设置syz-executor的选项，默认选项为去除重复覆盖率
	opts := &ExecOpts{
...
func MakeEnv(config *Config, pid int) (*Env, error) {
	...
	// config.Timeout默认为0
	if config.Timeout == 0 {
		// executor需要防止大多数的hangs，所以需要一个大的时间。
	...
	// 如果我们手动传入一个很小的时间(小于minTimeout的7s)，进程间的通讯(IPC)超时时间必须
	// 大于执行的时间
	...
	if config.Flags&FlagUseShmem != 0 {
		var err error
		// CreateMemMappedFile创建临时文件和一个共享内存地址的映射，inf存放临时文件，inmem存放共享内存地址
		// 共享内存大小为ExecBufferSize
		inf, inmem, err = osutil.CreateMemMappedFile(prog.ExecBufferSize)
		if err != nil {
			return nil, err
		}
		defer func() {
			// 最后会将inf和outf置为nil，如果不为nil，释放共享内存
			if inf != nil {
				osutil.CloseMemMappedFile(inf, inmem)
			}
		}()
		// 同inf一样，创建一个输出的共享内存，临时文件的文件名是不会一样的，虽然它们参数相同
		...
		// 如果不是用共享内存的方式，直接创建[]byte类型进行数据传递
	...
	// 作用英文注释已将讲的很明白了，将syz-executor的名字扩展为syz-executor+pid的形式，pid表示线程的后缀
	// 这样可以很容易的在输出日志中定位是哪一个syz-executor执行出了crash
